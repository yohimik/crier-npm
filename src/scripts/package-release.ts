...
async function main(options: PackageReleaseOptions = {}) {
  const version = options.version || process.env.CRIER_BINARY_VERSION
  if (!isVersion(version)) throw new Error('CRIER_BINARY_VERSION must be an exact semantic version')
  const tag = `v${version}`
  const api = `${options.api || 'https://api.github.com/repos/yohimik/crier/releases/tags'}/${encodeURIComponent(tag)}`
  ...
  try {
    response = await (options.fetch || makeFetchHappen as unknown as PackageReleaseOptions['fetch'])!(api, {
      ...
      headers: { accept: 'application/vnd.github+json', 'user-agent': '@dispat/crier release packager' }
    })
...
if (isMain(import.meta.url)) main().catch(error => { console.error(`crier npm packager: ${error.message}`); process.exitCode = 1 })
