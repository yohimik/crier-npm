...
export async function waitForNpm(version: string | undefined, options: WaitOptions = {}): Promise<void> {
  ...
  while (now() < deadline) {
    ...
    try {
      response = await request('https://registry.npmjs.org/@dispat%2fcrier', {
    ...
    if (metadata) {
      ...
      if (metadata.name === '@dispat/crier' && release?.name === '@dispat/crier' && release.version === version && release.dist?.tarball && release.dist.integrity) {
        log(`@dispat/crier@${version} is available in npm metadata`)
        return
      }
    ...
    log(`Waiting for @dispat/crier@${version}: ${reason}`)
    await pause(Math.min(interval, remaining))
  }
  throw new Error(`Timed out waiting for @dispat/crier@${version} after ${timeout}ms: ${reason}`)
}
