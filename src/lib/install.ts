...
export const BASE_URL = 'https://github.com/yohimik/crier/releases/download'
...
export function validateMetadata(release: unknown, key: string): ReleaseAsset {
  ...
  if (!record(release) || !isVersion(release.version) || release.tag !== `v${release.version}` ||
...
export async function validBinary(file: string, version: string, exec: BinaryExecutor = execute): Promise<boolean> {
  try {
    ...
    return new RegExp(`(?:^|\\n)crier ${version.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:\\s|$)`).test(stdout.trim())
  } catch { return false }
}
...
export async function install(options: InstallOptions = {}): Promise<{ target: string; installed: boolean }> {
  ...
  try {
    ...
    if (!await validBinary(temp, release.version, options.exec)) throw new Error(`downloaded executable did not report crier ${release.version}`)
    await rename(temp, target)
    options.log?.('info', 'installed crier', { version: release.version })
    return { target, installed: true }
  } finally {
    response?.body?.destroy()
    response?.crierCleanup?.()
