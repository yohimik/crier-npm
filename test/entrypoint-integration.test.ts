...
async function fixture(t: TestContext, nativeSource?: string) {
  const root = await mkdtemp(join(tmpdir(), 'crier entrypoint '))
  ...
  if (nativeSource) {
    ...
    await writeFile(join(root, 'release.json'), JSON.stringify({ version: '1.10.0', tag: 'v1.10.0', assets: {
...
test('launcher preserves literal arguments, working directory, environment and stream contents', unixOnly, async t => {
  const root = await fixture(t, `process.stdin.setEncoding('utf8'); let input=''; process.stdin.on('data', chunk => input+=chunk); process.stdin.on('end', () => {
    process.stdout.write(JSON.stringify({ args:process.argv.slice(2), cwd:process.cwd(), input, update:process.env.CRIER_TEST_ENV, custom:process.env.CRIER_TEST_VALUE }));
  ...
  const child = spawn(process.execPath, [join(root, 'build/bin/crier.js'), ...args], {
    cwd, env: { ...process.env, CRIER_TEST_ENV: '1', CRIER_TEST_VALUE: 'preserved' }, stdio: ['pipe', 'pipe', 'pipe']
  })
  let stdout = '', stderr = ''
...
test('launcher forwards SIGINT unchanged and exits when the child is interrupted', unixOnly, async t => {
  ...
  const child = spawn(process.execPath, [join(root, 'build/bin/crier.js')], { stdio: ['ignore', 'pipe', 'pipe'] })
...
test('launcher returns the original child termination signal', unixOnly, async t => {
  ...
  await assert.rejects(execute(process.execPath, [join(root, 'build/bin/crier.js')]), (error: unknown) => {
...
test('missing binary reports repair instructions without trying a download', async t => {
  ...
  await assert.rejects(execute(process.execPath, [join(root, 'build/bin/crier.js'), '--version']), (error: unknown) => {
...
test('npm-managed global update advice retains npm 12 script approval', async t => {
  const root = await fixture(t)
  await assert.rejects(execute(process.execPath, [join(root, 'build/bin/crier.js'), 'self-update']), (error: unknown) => {
    const failure = error as { code:number; stderr:string }
    assert.equal(failure.code, 2)
    assert.match(failure.stderr, /@latest --allow-scripts=@dispat\/crier/)
    assert.match(failure.stderr, /@1\.1\.0 --allow-scripts=@dispat\/crier/)
    return true
  })
})

test('postinstall verifies an existing binary and is silent on repeat installation', unixOnly, async t => {
  const root = await fixture(t, "process.stdout.write('crier 1.10.0\\n')")
...
test('postinstall failure is actionable and debug output remains on stderr', async t => {
  ...
  for (const debug of ['', '1']) {
    await assert.rejects(execute(process.execPath, [join(root, 'build/bin/postinstall.js')], { env: { ...process.env, CRIER_NPM_DEBUG: debug } }), (error: unknown) => {
      const failure = error as { code: number; stdout: string; stderr: string }
      assert.equal(failure.code, 1)
      assert.equal(failure.stdout, '')
