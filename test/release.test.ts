...
const metadata = (version='1.10.1') => ({ tag_name:`v${version}`, assets:platforms.map(([,name],i)=>({name,size:i+1,digest:`sha256:${String(i).repeat(64)}`})) })
...
test('generates exact release metadata without latest resolution', async t => {
  ...
  assert.match(url!,/v1\.10\.1$/); const got=JSON.parse(await fs.readFile(path.join(dir,'release.json'), 'utf8'))
...
test('release metadata generation fails closed', async t => {
  ...
  const previousVersion = process.env.CRIER_BINARY_VERSION
  delete process.env.CRIER_BINARY_VERSION
  try { await assert.rejects(main({dir}),/exact semantic/) } finally {
    if (previousVersion !== undefined) process.env.CRIER_BINARY_VERSION = previousVersion
  }
  ...
  await assert.rejects(main({version:'1.10.1',dir,fetch:async()=>response({tag_name:'v1.10.1'})}),/complete metadata/)
})
...
test('release metadata can use the workspace pin and canonical API without writing latest', async t => {
  ...
  try {
    ...
    assert.match(url, /^https:\/\/api\.github\.com\/.+v1\.10\.1$/)
  } finally {
...
test('publisher performs exactly one npm publish operation', async () => {
  const calls: string[][] = []
  await publish({ tarball:'/tmp/crier-cli.tgz', run:async args => { calls.push(args) } })
  assert.deepEqual(calls, [[
    'publish', '/tmp/crier-cli.tgz', '--access', 'public', '--provenance', '--tag', 'latest'
  ]])
})

test('publisher passes prerelease channels to npm without registry operations', async () => {
  const calls: string[][] = []
  await publish({ tarball:'/tmp/crier-cli.tgz', channel:'rc', run:async args => { calls.push(args) } })
...
test('publisher uses release outputs and propagates npm failures', async () => {
  ...
  process.env.DISPAT_OUTPUT_TARBALL = '/tmp/crier-cli.tgz'
