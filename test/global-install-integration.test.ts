...
test('packed global install recovers from npm 12 blocking its postinstall', { skip: process.platform === 'win32' }, async t => {
  const work = await mkdtemp(path.join(tmpdir(), "crier global ' $HOME $(nope) `nope` "))
  ...
  const body = Buffer.from("#!/bin/sh\nprintf 'crier 1.10.99\\n'\n")
  await writeFile(native, body)
  await chmod(native, 0o755)
  const key = platformKey()
  await writeFile(path.join(source, 'release.json'), JSON.stringify({ version:'1.10.99', tag:'v1.10.99', assets:{
  ...
  const installed = path.join(prefix, 'lib/node_modules/@dispat/crier')
  const command = path.join(prefix, 'bin/crier')
  ...
  await assert.rejects(execute(command, ['--version'], { cwd:work, timeout:30_000 }), error => {
    ...
    assert.match(repair, /@dispat\/crier\/build\/bin\/postinstall\.js'$/)
    return true
  })
  const mockFetch = path.join(work, 'mock-fetch.mjs')
  const loader = path.join(work, 'loader.mjs')
  await writeFile(mockFetch, `import { createReadStream } from 'node:fs'; export default async()=>({ok:true,status:200,statusText:'OK',headers:{get:n=>n.toLowerCase()==='content-length'?process.env.CRIER_FIXTURE_SIZE:null},body:createReadStream(process.env.CRIER_FIXTURE_BINARY)})\n`)
  await writeFile(loader, `import { pathToFileURL } from 'node:url'; export async function resolve(s,c,n){return s==='make-fetch-happen'?{url:pathToFileURL(process.env.CRIER_FIXTURE_FETCH).href,shortCircuit:true}:n(s,c)}\n`)
  const env = { ...process.env, NODE_OPTIONS:`--experimental-loader=${pathToFileURL(loader).href}`, CRIER_FIXTURE_FETCH:mockFetch,
    CRIER_FIXTURE_BINARY:native, CRIER_FIXTURE_SIZE:String(body.length) }
  await execute('/bin/sh', ['-c', repair], { cwd:work, env, timeout:30_000 })
  assert.match((await execute(command, ['--version'], { cwd:work, timeout:30_000 })).stdout, /^crier 1\.10\.99/m)
