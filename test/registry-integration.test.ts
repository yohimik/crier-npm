...
async function registry(t: TestContext) {
  const directory = await mkdtemp(join(tmpdir(), 'crier registry '))
  let document: RegistryDocument = { name: '@dispat/crier', versions: {}, 'dist-tags': {} }
  let publications = 0
  ...
  async function pack(version: string) {
    await writeFile(join(directory, 'package.json'), JSON.stringify({
      name: '@dispat/crier', version, description: 'Disposable registry integration fixture', files: ['README.md'],
...
test('publish entry point follows npm success and failure without registry reconciliation', async t => {
  ...
  const shim = await mkdtemp(join(tmpdir(), 'crier npm shim '))
  ...
  assert.match(result.stdout, /published @dispat\/crier/)
