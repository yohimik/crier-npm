...
async function fixture(t: TestContext, body = Buffer.from('native executable')) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'crier npm ')); t.after(() => fs.rm(dir, { recursive: true, force: true }))
  const release = { version: '1.10.0', tag: 'v1.10.0', assets: { 'linux-x64': { name: 'crier-linux-amd64', size: body.length, sha256: crypto.createHash('sha256').update(body).digest('hex') } } }
...
const goodExec: BinaryExecutor = async (_file, _args, options) => { assert.equal(options.env?.PATH, process.env.PATH); return { stdout: 'crier 1.10.0\n' } }

test('validates complete immutable metadata', () => {
  const release = { version:'1.0.0',tag:'v1.0.0',assets:{ 'linux-x64':{name:'crier-linux-amd64',size:1,sha256:'a'.repeat(64)} } }
...
test('streams, verifies, chmods and atomically installs', async t => {
  ...
  const result = await install({ packageDir:f.dir, platform:'linux', arch:'x64', release:f.release, exec:goodExec, download:async url => { assert.match(url, /v1\.10\.0/); const r=response(f.body); r.crierCleanup=()=>{cleaned=true}; return r } })
  ...
  assert.deepEqual(await fs.readdir(f.dir), ['crier-native','release.json'])
})

test('accepts a valid existing binary and replaces an invalid same-version binary', async t => {
  const f = await fixture(t); const target=path.join(f.dir,'crier-native'); await fs.writeFile(target,f.body)
...
test('preserves an existing binary and cleans temporary files on failures', async t => {
  const f=await fixture(t); const target=path.join(f.dir,'crier-native'); await fs.writeFile(target,'old')
...
test('rejects a downloaded executable reporting the wrong version', async t => {
  ...
  await assert.rejects(install({packageDir:f.dir,platform:'linux',arch:'x64',release:f.release,exec:async()=>({stdout:'crier 1.9.0'}),download:async()=>response(f.body)}),/did not report/)
...
test('reads packaged metadata and reports installation events at their severity', async t => {
  ...
  assert.deepEqual(events, ['debug: selected release asset', 'trace: starting binary download', 'info: installed crier'])
  events.length = 0
...
test('Windows temporary executable retains an exe suffix before validation', async t => {
  const f = await fixture(t)
  const release = { ...f.release, assets: { 'win32-arm64': { ...f.release.assets['linux-x64'], name: 'crier-windows-arm64.exe' } } }
  ...
  assert.equal(path.basename(result.target), 'crier-native.exe')
...
test('a directory at the binary path is preserved when replacement fails', async t => {
  const f = await fixture(t)
  const target = path.join(f.dir, 'crier-native')
