...
const release = { name: '@dispat/crier', version, dist: { tarball: 'https://registry.npmjs.org/@dispat/crier/-/bin-1.10.0.tgz', integrity: 'sha512-fixture' } }
const ready = { name: '@dispat/crier', versions: { [version]: release } }
...
test('npm readiness retries missing metadata and transient errors until the exact version is complete', async () => {
  const responses: (Response | Error)[] = [
    ...
    Response.json({ name: '@dispat/crier', versions: { '1.9.0': release } }),
    Response.json({ name: '@dispat/crier', versions: { [version]: { name: '@dispat/crier', version } } }),
    Response.json({ ...ready, name: 'wrong-package' }),
    Response.json({ name: '@dispat/crier', versions: { [version]: { ...release, name: 'wrong-package' } } }),
    Response.json({ name: '@dispat/crier', versions: { [version]: { ...release, version: '1.9.0' } } }),
    Response.json({ name: '@dispat/crier', versions: { [version]: { ...release, dist: { tarball: release.dist.tarball } } } }),
    Response.json(ready)
  ]
  let clock = 0
  ...
  await waitForNpm(version, {
    ...
    fetch: async (url, options) => {
      assert.equal(url, 'https://registry.npmjs.org/@dispat%2fcrier')
...
test('post-release check can run the compiled helper on supported Node versions', () => {
  const script = `
    import { waitForNpm } from './build/scripts/wait-for-npm.js'
